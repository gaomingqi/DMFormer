...
class ModulatedDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            glip_resize = T.RandomResize([480], max_size=720)
            ...
            for i, transf in enumerate(self._transforms.transforms):
                if i == 2:
                    glip_img, glip_target = glip_resize(img, target)
                    glip_img, glip_target = transf(glip_img, glip_target)
                    # glip_img = glip_imgtorch.stack(glip_img, dim=0)   # [T, 3, H, W]
                    glip_img = glip_img.unsqueeze(0)
        ...
        return img.unsqueeze(0), target, glip_img, glip_target
