...
class YTVOSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            # 1. photometric distort
            # 2. random select (or)
            #   2.0. random resize, check (object inside)
            #   2.1. random resize; random size crop; random resize; check (object inside) 
            # 3. ToTensor; Normalize

            # for glip, less challenging (only to tensor and normalise)
            visualise_transform = T.ToTensor()
            glip_resize = T.RandomResize([360], max_size=640)

            for i, transf in enumerate(self._transforms.transforms):
                if i == 3:
                    glip_imgs, glip_targets = glip_resize(imgs, target)
                    glip_imgs, glip_targets = transf(glip_imgs, glip_targets)
                    
                    originals, _ = glip_resize(imgs, None)
                    originals, _ = visualise_transform(originals, None)

                    originals = torch.stack(originals, dim=0)   # [T, 3, H, W]
                    originals = (originals*255).byte()
                    glip_imgs = torch.stack(glip_imgs, dim=0)   # [T, 3, H, W]
        ...
        return imgs, target, glip_imgs, glip_targets
