...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors, layer, 
                                       box_cls=None,
                                       token_logits=None,
        ...
        for per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors \
            ...
            boxlist.add_field("labels", per_class)
            boxlist.add_field("scores", torch.sqrt(per_box_cls))
            boxlist.add_field("indices", per_candidate_nonzeros)
            boxlist.add_field("layer", torch.tensor([layer]*len(per_class), dtype=torch.uint8).to(device))
            boxlist = boxlist.clip_to_image(remove_empty=False)
            boxlist = remove_small_boxes(boxlist, self.min_size)
            results.append(boxlist)
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        for idx, (b, c, a) in enumerate(zip(box_regression, centerness, anchors)):
            ...
            boxes, scores = self.forward_for_single_feature_map(b, c, a, idx, o, t, d, positive_map)
            dot_product_scores.append(scores)
        ...
        return boxlists, dot_product_scores
