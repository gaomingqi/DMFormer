...
# select confident boxes from predictions
def select_boxes(predictions, threshold=0.1):
...
class GeneralizedVLRCNN(nn.Module):
    ...
    def forward(self, 
        ...
        # split sentence into noun chunks
        doc = nlp(captions[0])
        ...
        if self.cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
            ...
            if captions is not None:
                ...
                for i, nc in enumerate(noun_chunks):
                    start_ids = [i for i, word_id in enumerate(tokenized.encodings[0].word_ids) if word_id == nc.start]
                    end_ids = [i for i, word_id in enumerate(tokenized.encodings[0].word_ids) if word_id == (nc.end-1)]
                    try:
                ...
                if len(positive_map) == 0:
                    ...
                    positive_map = {1: tokenized['attention_mask'][0].nonzero(as_tuple=True)[0].tolist()}
        ...
        if self.force_boxes:
            ...
            proposals, proposal_losses, fused_visual_features, scores = self.rpn(images, visual_features, targets, language_dict_features, positive_map,
        ...
        result = [select_boxes(box) for box in result]

        subject_label = torch.zeros([tokenized['attention_mask'][0].sum()]).to(device)

        return result, scores, shapes, positive_map, subject_label, noun_chunks
