...
def build_flickr_evaluator(cfg):
    evaluator = FlickrEvaluator(
        "/ssd1/gaomingqi/datasets/captions/flickr30k/flickr30k/", # Hard written!!
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    output = model(images, captions=captions, positive_map=positive_map_label_to_token, image_path=image_path)
                    output = [o.to(cpu_device) for o in output]
