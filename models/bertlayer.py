...
class BertEncoderLayer(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.attention = BertAttention(config,  True, True)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)

    def forward(self, language_feature, attention_mask):
        """
        language_feature: b x L x C (768)
        attention_mask: b x L
        """
        device = language_feature.device
        input_shape = language_feature.size()[:-1]
