...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, writer=None, epoch=None, header=None):
        i = 0
        if not header:
            header = ''
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if epoch is not None:
                    writer.add_scalar('training loss', self.meters['loss'].avg, epoch*len(iterable)+i)
            i += 1
            end = time.time()
...
def collate_fn(batch):
    ...
    if batch[2][0] is not None:    # glip_images
        batch[2] = nested_tensor_from_videos_list(batch[2], size_divisibility=32) 

    # if batch[4] is not None:    # original
    #     batch[4] = nested_tensor_from_videos_list(batch[4], size_divisibility=32) 
